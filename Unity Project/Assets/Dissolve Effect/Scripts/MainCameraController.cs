...
public class MainCameraController : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        else if (!Event.current.alt && Event.current.type == EventType.mouseDrag && Event.current.button == 1)
        {
            AngleY += Event.current.delta.x * FpsRotationSpeed;
            AngleX += Event.current.delta.y * FpsRotationSpeed;
        }

        if (Event.current.alt && Event.current.type == EventType.mouseDrag && Event.current.button == 1)
        {
            DoScrolling(Event.current.delta.y * ScrollingSpeed * -0.01f);
            //AngleY += Event.current.delta.x * FpsRotationSpeed;
            //AngleX += Event.current.delta.y * FpsRotationSpeed;
        }
        ...
        else if (Event.current.control && Event.current.type == EventType.ScrollWheel)
        {
